...
typedef enum
{
    ...
    SONG_TAOTAO = 5,         // เต่า (Taotao) (3 parts)
    SONG_BABY_SHARK = 6      // Baby Shark (2 parts)
} song_id_t;
...
static inline uint8_t calculate_checksum(const orchestra_message_t *msg)
{
    ...
    for (int i = 0; i < sizeof(orchestra_message_t) - 1; i++)
    { // -1 เพื่อไม่รวม checksum field
        sum += data[i];
    }
    return sum;
}
